...
type
  ...
  TSocketEvent = (seLookup, seConnecting, seConnect, seDisconnect, seListen,
		seAccept, seWrite, seRead, seNotify);
  ...
  TSocketNotifyEvent = procedure (Sender: TObject; Socket: TCustomWinSocket) of object;
	...
	protected
		...
		procedure DoOpen(NoEvent: Boolean = False);
    ...
    procedure Disconnect(Socket: TSocket; NoEvent: Boolean = False); virtual;
  ...
  public
	...
	private
    FNoEvent, FIsCmd: Boolean;
		FServerClientThread: TServerClientThread;
		FServerWinSocket: TServerWinSocket;
	public
		constructor Create(Socket: TSocket; ServerWinSocket: TServerWinSocket);
    procedure Disconnect(Socket: TSocket; NoEvent: Boolean = False); override;
		destructor Destroy; override;

		property ServerWinSocket: TServerWinSocket read FServerWinSocket;
		property ServerClientThread: TServerClientThread read FServerClientThread write FServerClientThread;
    property IsCmd: Boolean read FIsCmd write FIsCmd;
    property NoSendDisconnEvent: Boolean read FNoEvent write FNoEvent;
	end;

	TWinSocketNotifyEvent = procedure(AClient: TServerClientWinSocket) of object;
    ...
    FOnClientNotify: TSocketNotifyEvent;
		...
		function GetActiveControlConnections: Integer;
		function GetActiveThreads: Integer;
		function GetConnections(Index: Integer): TCustomWinSocket;
		function GetControlConnections(Index: Integer): TCustomWinSocket;
	...
	protected
		...
		procedure ClientNotify(Socket: TCustomWinSOcket); dynamic;
		...
		procedure ClientErrorEvent(Socket: TCustomWinSocket; ErrorEvent: TErrorEvent;
			var ErrorCode: Integer); dynamic;
	public
		procedure AddClient(AClient: TServerClientWinSocket; Control: Boolean = False);
		...
		property ActiveControlConnections: Integer read GetActiveControlConnections;
		...
		property ControlConnections[Index: Integer]: TCustomWinSocket read GetControlConnections;
		property ListControlConnections: TList read FControlConnections;
		property IdleThreads: Integer read GetIdleThreads;
    property ServerAcceptThread: TServerAcceptThread read FServerAcceptThread;
		...
		property OnClientNotify: TSocketNotifyEvent read FOnClientNotify write FOnClientNotify;
...
//8	TServerAcceptThread = class(TThread)
//8>>
	TServerAcceptThread = class(TMyThread)
//8<<
	private
...
//8	TServerClientThread = class(TThread)
//8>>
	TServerClientThread = class(TMyThread)
//8<<
  protected
	...
	protected
		...
		procedure ReActivate(ASocket: TServerClientWinSocket); virtual;
	...
	public
		...
		property ServerSocket: TServerWinSocket read FServerSocket;
		property Data: Pointer read FData write FData;
		property PlayEvent: TSimpleEvent read FEvent; 
	end;
  ...
  published
	...
	protected
		...
		property OnClientNotify: TSocketNotifyEvent index 4 read GetOnClientEvent
...
uses tServiceMain, MyForms, Consts, vSConnect;
...
{$ifdef linger}
  lngr.l_onoff := 1;
  lngr.l_linger := 3;
  CheckSocketResult(setsockopt(FSocket, SOL_SOCKET, SO_LINGER, Pointer(@lngr), SizeOf(lngr)), 'setsockopt');
{$endif}  
...
procedure TCustomWinSocket.DoOpen(NoEvent: Boolean = False);
begin
  DoSetASyncStyles;
  if (not NoEvent) then
...
procedure TCustomWinSocket.Disconnect(Socket: TSocket; NoEvent: Boolean = False);
begin
	Lock;
  try
...
begin
  try
    Dispatch(Message);
		...
		tServiceMain.Application.HandleException(Self);
  end;
end;
...
//---
  ...
  if Assigned(FServerWinSocket) then
  begin
    if FServerWinSocket.AsyncStyles <> [] then begin
      OnSocketEvent := FServerWinSocket.ClientEvent;
      OnErrorEvent := FServerWinSocket.ClientError;
    end
    else
      with FServerWinSocket do begin
        OnSocketEvent := ClientEvent;
        OnErrorEvent := ClientError;
      end;
  end;
  inherited Create(Socket);
end;

procedure TServerClientWinSocket.Disconnect(Socket: TSocket; NoEvent: Boolean = False);
begin
  if FAsyncStyles <> [] then begin
    if FIsCmd then Event(Self, seNotify);
  end
  else
    if FIsCmd and Assigned(FServerWinSocket) then
      FServerWinSocket.Event(Self, seNotify);
  inherited Disconnect(Socket, FNoEvent);
end;
...
begin
	...
	FControlConnections := TList.Create;
...
begin
	inherited Destroy;
	FControlConnections.Free;
	FConnections.Free;
	FActiveThreads.Free;
  FListLock.Free;
end;

procedure TServerWinSocket.AddClient(AClient: TServerClientWinSocket; Control: Boolean = False);
begin
	FListLock.Enter;
	try
		if Control then begin
			if FControlConnections.IndexOf(AClient) < 0 then
				FControlConnections.Add(AClient);
		end
		else begin
...
begin
	...
	try
		...
		FControlConnections.Remove(AClient);
	finally
		FListLock.Leave;
	end;
end;
...
function TServerWinSocket.GetActiveControlConnections: Integer;
begin
	Result := FControlConnections.Count;
end;
...
function TServerWinSocket.GetControlConnections(Index: Integer): TCustomWinSocket;
begin
	Result := FControlConnections[Index];
end;
...
begin
    ...
    try
      ...
      if Assigned(ClientSocket) then begin
        if ClientSocket.ServerClientThread <> nil then
          ClientSocket.ServerClientThread.Terminate;
        ClientSocket.Free;
      end;
    end;
	finally
...
procedure TServerWinSocket.Disconnect(Socket: TSocket; NoEvent: Boolean = False);
var
 P: Pointer;
begin
	Lock;
	try
        ...
        P := FActiveThreads.Last;
				with TServerClientThread(P) do
				begin
...
procedure TServerWinSocket.ClientNotify(Socket: TCustomWinSocket);
begin
	if Assigned(FOnClientNotify) then FOnClientNotify(Self, Socket);
end;
...
begin
	...
	begin
		...
		FClientSocket.ServerClientThread := Self;
		FEvent.SetEvent;
	end;
end;
...
begin
	...
	begin
		tServiceMain.Application.ShowException(FException);
	end else
...
begin
  case Index of
    ...
    4: Result := FServerSocket.OnClientNotify;
  end;
end;
...
begin
  case Index of
    ...
    4: FServerSocket.OnClientNotify := Value;
	end;
end;
