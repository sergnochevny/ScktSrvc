...
procedure TServiceApplication.RegisterServices(Install, Silent: Boolean);
  ...
  procedure InstallService(Service: TService; tSvcMgr: Integer);
  var
  ...
  begin
    ...
    begin
      ...
      Svc := CreateService(tSvcMgr, PChar(Name), PChar(DisplayName),
        SERVICE_ALL_ACCESS, GetNTServiceType, GetNTStartType, GetNTErrorSeverity,
        PChar(Path+' -s'), PChar(LoadGroup), PTag, PChar(GetNTDependencies),
              //5 ^^^^^^
        PSSN, PChar(Password));
      TagID := TmpTagID;
      if Svc = 0 then
  ...
  procedure UninstallService(Service: TService; tSvcMgr: Integer);
  var
    Svc: Integer;
  begin
    with Service do
    begin
      ...
      Svc := OpenService(tSvcMgr, PChar(Name), SERVICE_ALL_ACCESS);
...
begin
  ...
  try
    for i := 0 to ComponentCount - 1 do
      ...
      try
        if Install then
          InstallService(TService(Components[i]), tSvcMgr) else
          UninstallService(TService(Components[i]), tSvcMgr)
      except
        on E: Exception do
        begin
          Success := False;
